...
class WorkspaceMembership(Base):
    ...
    role = Column(String(50), nullable=False, default="Viewer") # String to avoid complex enum casting
...
class Workspace(Base):
    ...
    pipelines = relationship("Pipeline", back_populates="workspace", cascade="all, delete-orphan")
...
class CopilotMessage(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    session_id = Column(UUID(as_uuid=True), ForeignKey("copilot_sessions.id", ondelete="CASCADE"), index=True, nullable=False)
    sender = Column(String(50), nullable=False) # 'user' or 'ai'
    text = Column(Text, nullable=False)
    card_type = Column(String(50), nullable=True)
    card_data = Column(JSONB().with_variant(JSON(), "sqlite"), nullable=True, default={})
    created_at = Column(DateTime(timezone=True), default=datetime.utcnow)
