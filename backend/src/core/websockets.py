...
class ConnectionManager:
    ...
    async def authenticate_and_connect(self, websocket: WebSocket, token: str) -> Optional[Tuple[str, str]]:
        ...
        if not workspace_id:
            logger.warning(f"WebSocket connect: user {user_id} does not belong to any workspace")
            await websocket.close(code=4008)
            return None
        ...
        logger.info(f"WebSocket client connected: user={user_id}, workspace={workspace_id}")
    ...
    async def broadcast_to_room(self, room: str, event_type: str, payload: dict):
        ...
        for socket in list(self.rooms[room]):
            try:
                ...
                logger.warning(f"WebSocket broadcast failure to socket in room {room}: {e}. Evicting.")
    ...
    async def broadcast_to_workspace(self, workspace_id: str, event_type: str, payload: dict):
