...
from sqlalchemy import Column, String, Integer, ForeignKey, DateTime, Text, Boolean, CheckConstraint, Index, JSON
...
class Pipeline(Base):
    ...
    dataset_id = Column(UUID(as_uuid=True), ForeignKey("datasets.id", ondelete="SET NULL"), index=True, nullable=True)
    name = Column(String(255), nullable=False)
    description = Column(String, nullable=True)
    status = Column(String(50), nullable=False, default="Idle") # 'Active', 'Paused', 'Idle'
    cron_schedule = Column(String(100), nullable=True)
    run_configuration = Column(JSONB().with_variant(JSON(), "sqlite"), nullable=True, default={})
    ...
    __table_args__ = (
        Index("idx_pipeline_workspace_status", "workspace_id", "status"),
    )
...
class PipelineRun(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    pipeline_id = Column(UUID(as_uuid=True), ForeignKey("pipelines.id", ondelete="CASCADE"), index=True, nullable=False)
    status = Column(String(50), nullable=False, default="queued") # 'queued', 'running', 'completed', 'failed', 'cancelled'
    triggered_by = Column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"), index=True, nullable=True)
    ...
    __table_args__ = (
        ...
        Index("idx_pipelinerun_pipeline_status_created", "pipeline_id", "status", "created_at"),
    )

    # Relationships
    pipeline = relationship("Pipeline", back_populates="runs")
    agent_metrics = relationship("AgentMetrics", back_populates="run", cascade="all, delete-orphan")
