...
async def broadcast_and_log(
    ...
    try:
        ...
        log_entry = ActivityLog(
            workspace_id=datetime.utcnow()
            if not workspace_id
            else workspace_id,  # UUID check handled by DB conversion
        ...
        await ws_manager.broadcast_to_workspace(
            workspace_id, "activity.created", payload
        )
        ...
        await ws_manager.broadcast_to_workspace(
            workspace_id, "pipeline.log", log_payload
        )

        # 5. Direct socket fallback
        if ws:
            try:
                await ws.send_json({"type": "log", "agent": agent, "message": message})
            except Exception:
                pass
...
async def broadcast_agent_telemetry(
    ...
    await ws_manager.broadcast_to_workspace(
        workspace_id, "agent.status.changed", {"agent": agent, "status": status}
    )
    # agent.queue.updated
    await ws_manager.broadcast_to_workspace(
        workspace_id, "agent.queue.updated", {"agent": agent, "queueSize": queue_size}
    )
...
async def run_extraction_pipeline(project_id: str, ws: WebSocket | None = None):
    ...
    try:
        ...
        if ws:
            try:
                await ws.send_json(
                    {"type": "status", "agent": "ingestion", "status": "running"}
                )
            except Exception:
                pass
        ...
        if ws:
            try:
                await ws.send_json(
                    {"type": "status", "agent": "ingestion", "status": "completed"}
                )
        ...
        if ws:
            try:
                await ws.send_json(
                    {"type": "status", "agent": "ocr", "status": "running"}
                )
        ...
        if ws:
            try:
                await ws.send_json(
                    {"type": "status", "agent": "ocr", "status": "completed"}
                )
        ...
        if ws:
            try:
                await ws.send_json(
                    {"type": "status", "agent": "extractor", "status": "running"}
                )
        ...
        if ws:
            try:
                await ws.send_json(
                    {"type": "status", "agent": "extractor", "status": "completed"}
                )
        ...
        if ws:
            try:
                await ws.send_json(
                    {"type": "status", "agent": "schema", "status": "running"}
                )
        ...
        if ws:
            try:
                await ws.send_json(
                    {"type": "status", "agent": "schema", "status": "completed"}
                )
        ...
        if ws:
            try:
                await ws.send_json(
                    {"type": "status", "agent": "validator", "status": "running"}
                )
        ...
        if ws:
            try:
                await ws.send_json(
                    {"type": "status", "agent": "validator", "status": "completed"}
                )
        ...
        if ws:
            try:
                await ws.send_json(
                    {"type": "status", "agent": "cleaner", "status": "running"}
                )
        ...
        if ws:
            try:
                await ws.send_json(
                    {"type": "status", "agent": "cleaner", "status": "completed"}
                )
        ...
        if run:
            ...
            run.duration_seconds = int(
                (
                    datetime.utcnow() - (run.started_at or datetime.utcnow())
                ).total_seconds()
            )
        ...
        await ws_manager.broadcast_to_workspace(
            ...
            "pipeline.completed",
            {
                ...
                "durationSeconds": run.duration_seconds if run else 0,
            },
        )
        ...
        if workspace_id:
            await ws_manager.broadcast_to_workspace(
                workspace_id,
                "pipeline.failed",
                {"pipelineId": pipeline_id, "runId": run_id, "errorMessage": str(e)},
            )

        if ws:
            try:
                await ws.send_json(
                    {"type": "failed", "message": f"Pipeline execution error: {str(e)}"}
                )
            except Exception:
                pass
