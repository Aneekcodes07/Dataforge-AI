...
import { Mail, Lock, Eye, EyeOff, Zap, ArrowRight, Loader2, User } from 'lucide-react';
...
export default function SignupPage() {
  ...
  const passwordStrength = (() => {
    ...
    return { level: 3, label: 'Good', color: '#FFB347' };
  })();
      ...
      <div className="hidden lg:flex lg:w-[48%] relative overflow-hidden items-center justify-center bg-[#050505] border-r border-white/[0.04]">
        ...
        <div className="absolute inset-6 border border-white/[0.02] pointer-events-none flex flex-col justify-between p-6">
          <div className="flex justify-between text-[10px] font-mono text-text-tertiary">
            <span>[DATAFORGE_SYS_REG]</span>
            <span>SYS_GATE_02 // REGISTRATION_LAYER</span>
          </div>
          <div className="flex justify-between text-[10px] font-mono text-text-tertiary">
            <span>LOC_COORD_37.7749_-122.4194</span>
            <span>REG_NODE_INIT</span>
          </div>
        </div>
        ...
        <div className="relative z-10 max-w-[480px] px-12">
          ...
          <h2 className="text-2xl font-bold font-mono uppercase text-text-primary tracking-wide mb-4">
            [JOIN_OPERATIONS_NETWORK]
          </h2>
          <p className="text-text-secondary font-mono text-xs leading-relaxed mb-8">
            Deploy and scale containerized data pipelines with AI-guided extractors. Get full access to agent hyperparameters, custom schedulers, and pipeline debugging logs.
          </p>

          <div className="grid grid-cols-2 gap-3 font-mono">
            {[
              { icon: '🚀', title: 'SYS_UNLIMITED_NODES', desc: 'No workspace caps' },
              { icon: '🤖', title: 'SYS_ACTIVE_AGENTS', desc: '07 pipeline agents' },
              { icon: '📊', title: 'SYS_EDA_ENGINE', desc: 'Full profile diagnostics' },
              { icon: '🔒', title: 'SYS_SOC2_CERT', desc: 'Strict layer isolation' },
            ].map((perk) => (
              <div key={perk.title} className="border border-white/[0.04] bg-[#0D0D0D] p-3 rounded-[4px] text-left flex flex-col justify-between min-h-[72px]">
                <div className="flex justify-between items-center">
                  <span className="text-[9px] font-bold text-text-secondary">{perk.title}</span>
                  <span className="text-xs">{perk.icon}</span>
                </div>
                <span className="text-[10px] text-text-tertiary mt-1">{perk.desc}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        >
          ...
          <motion.div variants={staggerItem} className="border-b border-white/[0.04] pb-4 mb-6 text-left">
            <h1 className="font-mono text-sm font-bold uppercase tracking-wider text-text-primary">
              [CREATE_OPERATIONS_PROFILE]
            </h1>
            <p className="text-xs text-text-secondary mt-1">
              Configure system credentials for your data pipeline environment.
            </p>
          </motion.div>
          ...
          {error && (
            ...
            >
              ...
              <div className="flex-1 text-left">
                <span className="font-bold">[ERR_REG_FAILURE]:</span> {error}
              </div>
            </motion.div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="mt-2 flex items-center gap-2">
                  <div className="flex-1 flex gap-1">
                    {[1, 2, 3, 4].map((level) => (
                      <div
                        key={level}
                        className="h-1 flex-1 transition-all duration-300 rounded-[1px]"
                        style={{
                          backgroundColor:
                            level <= passwordStrength.level ? passwordStrength.color : 'rgba(255,255,255,0.04)',
                        }}
                      />
                    ))}
                  </div>
                  <span className="text-[9px] font-bold font-mono tracking-wide uppercase" style={{ color: passwordStrength.color }}>
            ...
            <motion.div variants={staggerItem} className="space-y-2 text-left font-mono">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={cn(
                    'input-base pl-10 rounded-[4px] font-mono text-xs',
              ...
              {confirmPassword && password !== confirmPassword && (
                <p className="text-[10px] text-danger font-mono font-bold mt-1 text-left">[ERR_PASSWORD_MISMATCH]</p>
              )}
            </motion.div>

            {/* Terms */}
            <motion.div variants={staggerItem} className="flex items-start gap-2.5 pt-1 text-left font-mono">
              <input
                type="checkbox"
                checked={agreed}
                onChange={(e) => setAgreed(e.target.checked)}
                className="mt-0.5 w-4 h-4 rounded-[2px] border-white/20 bg-white/5 text-accent focus:ring-accent/30 cursor-pointer"
                id="signup-terms"
              />
              <label htmlFor="signup-terms" className="text-xs text-text-secondary leading-normal select-none">
                I authorize agreement to the{' '}
                <a href="#" className="text-accent hover:text-[#EA580C] font-bold">[TERMS_OF_SERVICE]</a>
                {' '}and{' '}
                <a href="#" className="text-accent hover:text-[#EA580C] font-bold">[PRIVACY_POLICY]</a>
              </label>
            </motion.div>
            ...
            <motion.div variants={staggerItem} className="pt-2">
              <button
                type="submit"
                disabled={isLoading || !agreed || (!!confirmPassword && password !== confirmPassword)}
                className={cn(
          ...
          <motion.p variants={staggerItem} className="text-center mt-8 text-xs text-text-secondary font-mono">
            Already have registered profile?{' '}
            <Link to="/login" className="text-accent hover:text-[#EA580C] font-bold transition-colors">
              [INITIATE_SESSION]
            </Link>
          </motion.p>
        </motion.div>
